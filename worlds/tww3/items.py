...
from .filler_item_manager import fillerItemManager
...
def createAllItems(world: TWW3World) -> None:
    ...
    for _ in range(len(world.locations) - len(pool)):
        name = item_manager.roll_for_item()
        item_id = world.item_name_to_id[name]
        item = TWW3Item(name, item_table[item_id].classification, item_id, player=world.player)
        pool.append(item)
